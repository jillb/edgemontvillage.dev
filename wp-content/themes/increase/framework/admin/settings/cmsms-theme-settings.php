...
function cmsms_form_field_fn($args = array()) {
	...
	case 'upload':
		...
		echo '<input id="' . $id . '" name="' . $cmsms_option_name . '[' . $id . ']" type="hidden" class="custom_upload_image" value="' . ((isset($options[$id]) && $options[$id] != '' && $options[$id] != $std) ? $options[$id] : $std) . '" />' . 
		'<img src="' . $image . '" id="' . $id . '_preview" class="custom_preview_image" alt="" />' . 
		'<br />' . 
